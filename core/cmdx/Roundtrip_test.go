...
// roundTrip is a helper that encodes markdown -> CMDX -> markdown
// and compares the ASTs of the original and decoded output.
func roundTrip(t *testing.T, input []byte) {
	...
	require.True(t, equal, "round-trip mismatch:\noriginal:\n%s\nencoded:\n%s\ndecoded:\n%s\ndiff:\n%s",
		string(input), string(encoded), string(decoded), diff)
}
...
func TestRoundTrip_codeBlockWithAtSlashCODE(t *testing.T) {
...
func TestRoundTrip_empty(t *testing.T) {
	...
	equal, _, err := CompareASTs([]byte(""), decoded)
	require.NoError(t, err)
...
// --- Phase 3: Round-trip tests with domain blocks ---

func TestRoundTrip_apiDocsFullPipeline(t *testing.T) {
	// Full round-trip of api_docs.md with all encoder/decoder passes enabled.
	roundTrip(t, readTestdata("api_docs.md"))
}

func TestRoundTrip_mixedContent(t *testing.T) {
	// Document with KV tables, plain tables, headings, paragraphs, code.
...
// --- Additional round-trip tests for coverage gaps ---

func TestRoundTrip_indentedCodeBlock(t *testing.T) {
	// Indented code block (4 spaces) should round-trip as fenced code block.
...
func TestEncode_githubNoteCallout(t *testing.T) {
	// GitHub-style [!NOTE] → @NOTE{} → > **Note:** ... (format changes, not round-trip)
	input := []byte("> [!NOTE]\n> This is a GitHub-style note.\n")
	encoded, err := Encode(input)
	require.NoError(t, err)
	output := string(encoded)
	assert.Contains(t, output, "@NOTE{This is a GitHub-style note.}")

	decoded, err := Decode(encoded)
	require.NoError(t, err)
	assert.Contains(t, string(decoded), "> **Note:** This is a GitHub-style note.")
}

func TestEncode_githubWarningCallout(t *testing.T) {
	// GitHub-style [!WARNING] → @WARN{} → > **Warning:** ... (format changes, not round-trip)
	input := []byte("> [!WARNING]\n> This is a GitHub-style warning.\n")
	encoded, err := Encode(input)
	require.NoError(t, err)
	output := string(encoded)
	assert.Contains(t, output, "@WARN{This is a GitHub-style warning.}")

	decoded, err := Decode(encoded)
	require.NoError(t, err)
	assert.Contains(t, string(decoded), "> **Warning:** This is a GitHub-style warning.")
}

func TestEncode_apiDocsMatchesExpected(t *testing.T) {
	// Encoder output for api_docs.md should match api_docs.cmdx.
	// Note: This is an exact match test that validates the full pipeline.
	// It depends on dictionary determinism and domain detection correctness.
	input := readTestdata("api_docs.md")
	expected := readTestdata("api_docs.cmdx")
	encoded, err := Encode(input)
	require.NoError(t, err)
	// Compare trimmed output to handle trailing newline differences.
	assert.Equal(t, strings.TrimSpace(string(expected)), strings.TrimSpace(string(encoded)),
		"encoded output should match expected api_docs.cmdx")
}
