...
func dumpNode(buf *strings.Builder, n ast.Node, source []byte, depth int) {
	...
	case *ast.TextBlock:
		// Skip empty TextBlocks that are artifacts of link reference definitions.
		// Goldmark consumes link ref defs as metadata but leaves behind empty
		// TextBlock nodes in the AST.
		if n.FirstChild() == nil {
			return
		}
