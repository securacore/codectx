...
func FuzzRoundTrip(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, input []byte) { //nolint:thelper
		...
		// be round-tripped through compact markdown.
...
func hasSerializationAmbiguity(input []byte) bool {
	...
	// Compact markdown doesn't support blockquotes, code blocks, etc. inside list items.
	...
	// Compact markdown stores table cells as plain text, losing inline markup.
	...
	// producing any visible output, so they can't round-trip.
	hasLinkRefDef := false
