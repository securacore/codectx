...
- **Compiled output IS a package.** The `.codectx/` directory follows the same package format (package.yml + directories) as every other package. Its `package.yml` is the unified data map that merges all active documentation. This means the compiled output is self-describing and can be consumed by any tool that understands the package format. Alternative considered: custom output format (rejected; introduces a second format that tools must understand). Alternative considered: flat file dump (rejected; loses the data map, dependencies, and loading metadata).
