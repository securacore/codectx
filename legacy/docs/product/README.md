...
|---|---|---|---|
| **Source manifest** | `docs/manifest.yml` | Indexes your source documentation. Lists entries with file paths relative to `docs/`. Also present in each installed package at `docs/packages/[name]@[author]/manifest.yml`. | You (or `codectx new`). Auto-updated by `codectx compile` when new files are discovered. |
